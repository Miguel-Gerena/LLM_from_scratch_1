...
def pre_tokenize(pattern:str, filename:str, endLine:bool=True) -> str:
...
def merge(indices: List[int], pair: Tuple[(int, int)], new_index: int, locations:OD[int, list], isdebug:bool =True) -> Tuple[List[int], OD[int, list]]:
    ...
    for location in locations_list:
        ...
        if location + 1 < len(indices) and (indices[location], indices[location + 1]) == pair:
            ...
            last_index = location + 2  # found a pair
        else:
            new_indices.append(indices[location])
            new_locations.append(len(new_indices) - 1)
            last_index = location + 1  # not a pair check next
    ...
    if new_locations: 
        if isdebug:
            for location in new_locations:
                assert pair[0] == new_indices[location]
